"""
荧光颗粒与组织染色检测器 v5.1 (Streamlit Web版)
...
def ensure_sam_model():
    """检查并提示SAM模型状态"""
    if not SAM_AVAILABLE:
        return False, "SAM 库未安装。本地使用请运行：pip install segment-anything"
    if not os.path.exists(SAM_CHECKPOINT_PATH):
        msg = (
            f"SAM 模型文件未找到（{SAM_CHECKPOINT_NAME}, ~375MB）。\n\n"
            f"**下载方式：**\n"
            f"1. 官方下载：https://github.com/facebookresearch/segment-anything#model-checkpoints\n"
            f"2. 放置到：`{SAM_CHECKPOINT_PATH}`\n\n"
            f"Streamlit Cloud 用户：建议将模型文件放入仓库根目录，并在代码中修改路径。"
        )
        return False, msg
    return True, "就绪"

def sam_refine_mask(img_bgr, coarse_mask, checkpoint_path):
    """
    用SAM精修传统阈值法得到的粗略掩膜。
    逻辑：SAM生成全图对象掩膜 → 只保留与粗略掩膜重叠>50%的对象 → 合并为精修掩膜。
    """
    ...
    for m in sam_masks:
        ...
        # 计算SAM掩膜与粗略阳性掩膜的重叠率
        overlap = cv2.bitwise_and(seg, coarse_mask)
        overlap_ratio = np.sum(overlap) / (np.sum(seg) + 1e-6)
        # 若SAM对象主要落在传统阳性区内，保留其精确边界
        if overlap_ratio > 0.5:
            refined = cv2.bitwise_or(refined, seg)
            kept += 1

    del sam, mask_generator, sam_masks
    gc.collect()
    return refined, kept
...
def init_state():
    defaults = {
        ...
        'uploaded_files': None, 'current_results': [], 'has_run': False, 'current_idx': 0,
        # 荧光颗粒
        'fluo_preset': '通用荧光颗粒', 'fluo_algos': ['LoG 斑点检测', '局部最大值'],
        'fluo_sens': 50, 'fluo_size': 8, 'fluo_unif': 20, 'fluo_dist': 6, 'fluo_color': '红色',
        ...
        'oilred_min_area': 30, 'oilred_watershed': True, 'oilred_use_sam': False,
        # ALP
        'alp_cell': 'MC3T3-E1', 'alp_mag': '10x',
        'alp_thresh': 135, 'alp_morph': 2, 'alp_min_area': 100, 'alp_use_sam': False,
        # 茜素红
        'alizarin_cell': 'MC3T3-E1', 'alizarin_mag': '10x',
        'alizarin_r': 120, 'alizarin_rg': 11, 'alizarin_rb': 11,
        'alizarin_morph': 2, 'alizarin_min_area': 150, 'alizarin_use_sam': False,
    }
...
def reset_fluo_v33():
    ...
    st.session_state.fluo_sens = 50; st.session_state.fluo_size = 8
    st.session_state.fluo_unif = 20; st.session_state.fluo_dist = 6
    st.session_state.fluo_color = '红色'; st.session_state.fluo_preset = '通用荧光颗粒'
    st.rerun()

def apply_oilred_ai():
    rec = AIRecommender.recommend('oilred', st.session_state.oilred_cell, st.session_state.oilred_mag)
    desc = rec.pop('desc', ''); st.toast(f"AI推荐: {desc}", icon="🤖")
...
def reset_oilred():
    ...
    st.session_state.oilred_use_sam = False
    st.rerun()
...
def reset_alp():
    ...
    st.session_state.alp_use_sam = False
    st.rerun()
...
def reset_alizarin():
    ...
    st.session_state.alizarin_use_sam = False
    st.rerun()


# ==================== SAM 优劣势说明组件 ====================
def show_sam_info():
    with st.expander("📖 SAM AI 分割模式说明（点击展开）", expanded=False):
        st.markdown("""
        **SAM (Segment Anything Model)** 是 Meta 发布的零样本图像分割模型，可作为传统阈值法的精修工具。

        | 维度 | 说明 |
        |------|------|
        | **原理** | 先用传统颜色阈值得到粗略阳性区域，再用 SAM 提取该区域内的精确对象边界 |
        | **优势** | ① 边界精度显著优于传统阈值+形态学；② 对粘连脂滴/结节的分离效果更好；③ 无需人工标注，零样本即用 |
        | **劣势** | ① 模型文件大（~375MB），首次加载慢；② 需要较强 CPU/GPU，分析速度比传统法慢 3–5 倍；③ 可能把背景杂质也分割出来，需配合传统阈值过滤；④ Streamlit Cloud 免费版内存吃紧，建议本地使用 |
        | **适用场景** | 油红O脂滴密集粘连、茜素红结节边缘模糊、ALP弥散矿化区域需要精确面积统计时 |
        | **不适用场景** | 极小颗粒（<5px）、荧光点状颗粒、需要极速批量处理（>50张）时 |

        **部署提示：**
        - 本地使用：运行 `pip install segment-anything`，下载 `sam_vit_b_01ec64.pth` 放到 `~/.cache/segment_anything/`
        - Streamlit Cloud：可将模型文件放入仓库根目录，修改代码中 `SAM_CHECKPOINT_PATH` 指向该文件
        """)


# ==================== Sidebar ====================
st.sidebar.title("🔬 染色检测器 v5.1")
st.sidebar.caption("Streamlit 在线版 | 四模块 | 可选SAM精修")
...
# SAM 全局状态检查
sam_ready, sam_msg = ensure_sam_model()
...
if module == "荧光颗粒计数":
    ...
    with c2:
        ...
        # SAM 开关
        st.checkbox("✨ 启用 SAM AI 精修边界", key='oilred_use_sam',
                    help="用SAM精修传统阈值结果，边界更精确，但分析更慢",
                    disabled=not sam_ready)
        if st.session_state.oilred_use_sam and not sam_ready:
            st.warning(sam_msg)
    ...
    with c2:
        ...
        st.checkbox("✨ 启用 SAM AI 精修边界", key='alp_use_sam',
                    help="用SAM精修传统阈值结果，边界更精确，但分析更慢",
                    disabled=not sam_ready)
        if st.session_state.alp_use_sam and not sam_ready:
            st.warning(sam_msg)
    ...
    with c2:
        ...
        st.checkbox("✨ 启用 SAM AI 精修边界", key='alizarin_use_sam',
                    help="用SAM精修传统阈值结果，边界更精确，但分析更慢",
                    disabled=not sam_ready)
        if st.session_state.alizarin_use_sam and not sam_ready:
            st.warning(sam_msg)
        ...
        for i, f in enumerate(uploaded_files):
            ...
            try:
                ...
                if module == "荧光颗粒计数":
                    ...
                    res = analyze_oil_red_o(img, params)
                    # SAM 精修
                    if st.session_state.oilred_use_sam and sam_ready:
                        with st.spinner("SAM AI 精修中..."):
                            refined_mask, kept = sam_refine_mask(img, res['mask'], SAM_CHECKPOINT_PATH)
                            res['mask'] = refined_mask
                            res['sam_kept_objects'] = kept
                            # 用精修后的掩膜重新统计
                            ...
                            for j in range(1, num_labels):
                                ...
                                if area >= params['min_droplet_area']:
                                    droplets.append({'id': j, 'area': int(area), 'centroid': (float(centroids[j][0]), float(centroids[j][1]))})
                            res['count'] = len(droplets)
                            res['droplets'] = droplets
                            res['avg_droplet_area'] = round(np.mean([d['area'] for d in droplets]), 1) if droplets else 0
                            res['positive_pixels'] = int(np.sum(refined_mask > 0))
                            res['area_ratio'] = res['positive_pixels'] / (img.shape[0] * img.shape[1]) * 100.0
                    ...
                    res = analyze_mineralization(img, params)
                    if st.session_state.alp_use_sam and sam_ready:
                        with st.spinner("SAM AI 精修中..."):
                            refined_mask, kept = sam_refine_mask(img, res['mask'], SAM_CHECKPOINT_PATH)
                            res['mask'] = refined_mask
                            res['sam_kept_objects'] = kept
                            ...
                            for j in range(1, num_labels):
                                area = stats[j, cv2.CC_STAT_AREA]
                                if area >= params['min_nodule_area']:
                                    nodules.append({'id': j, 'area': int(area), 'centroid': (float(centroids[j][0]), float(centroids[j][1]))})
                            res['nodule_count'] = len(nodules)
                            res['nodules'] = nodules
                            res['avg_nodule_area'] = round(np.mean([n['area'] for n in nodules]), 1) if nodules else 0
                            res['positive_pixels'] = int(np.sum(refined_mask > 0))
                            res['area_ratio'] = res['positive_pixels'] / (img.shape[0] * img.shape[1]) * 100.0
                    ...
                    res = analyze_mineralization(img, params)
                    if st.session_state.alizarin_use_sam and sam_ready:
                        with st.spinner("SAM AI 精修中..."):
                            refined_mask, kept = sam_refine_mask(img, res['mask'], SAM_CHECKPOINT_PATH)
                            res['mask'] = refined_mask
                            res['sam_kept_objects'] = kept
...
# --- 染色模块结果显示 ---
    ...
    if img_rgb_display is not None:
        ...
        if module == "油红O脂滴":
            ...
            if 'sam_kept_objects' in res:
                st.info(f"🤖 SAM 精修：保留了 {res['sam_kept_objects']} 个精确边界对象")
        else:
...
st.caption("提示：内存优化已启用，单图最大边长限制 2048px。SAM模式分析较慢，请耐心等待。")
